import type { Lesson } from '../types';

const lesson: Lesson = {
  id: 'lesson-21',
  theme: 'at-home-2',
  icon: '🏠',
  title: {
    chinese: '在家裡（二）',
    english: 'At Home (2)',
  },
  vocabulary: [
    { id: 'vocab-01', character: '客廳', english: 'living room' },
    { id: 'vocab-02', character: '房間', english: 'bedroom' },
    { id: 'vocab-03', character: '廚房', english: 'kitchen' },
    { id: 'vocab-04', character: '浴室', english: 'bathroom' },
    { id: 'vocab-05', character: '門', english: 'door' },
    { id: 'vocab-06', character: '窗', english: 'window' },
    { id: 'vocab-07', character: '樓', english: 'floor, building' },
    { id: 'vocab-08', character: '住', english: 'live' },
    { id: 'vocab-09', character: '花園', english: 'garden' },
    { id: 'vocab-10', character: '陽台', english: 'balcony' },
    { id: 'vocab-11', character: '樓梯', english: 'stairs' },
    { id: 'vocab-12', character: '傢俱', english: 'furniture' },
  ],
  sentences: [
    {
      id: 'sent-01',
      chinese: '爸爸在客廳看電視。',
      english: 'Dad watches TV in the living room.',
      words: [
        { character: '爸爸', english: 'dad' },
        { character: '在', english: 'at' },
        { character: '客廳', english: 'living room', isVocab: true },
        { character: '看', english: 'watch' },
        { character: '電視', english: 'TV' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-02',
      chinese: '我的房間很大。',
      english: 'My bedroom is very big.',
      words: [
        { character: '我的', english: 'my' },
        { character: '房間', english: 'bedroom', isVocab: true },
        { character: '很', english: 'very' },
        { character: '大', english: 'big' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-03',
      chinese: '媽媽在廚房煮飯。',
      english: 'Mom cooks in the kitchen.',
      words: [
        { character: '媽媽', english: 'mom' },
        { character: '在', english: 'at' },
        { character: '廚房', english: 'kitchen', isVocab: true },
        { character: '煮飯', english: 'cook' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-04',
      chinese: '請關門。',
      english: 'Please close the door.',
      words: [
        { character: '請', english: 'please' },
        { character: '關', english: 'close' },
        { character: '門', english: 'door', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-05',
      chinese: '打開窗，好涼快。',
      english: 'Open the window, so cool.',
      words: [
        { character: '打開', english: 'open' },
        { character: '窗', english: 'window', isVocab: true },
        { character: '，', english: '', isPunctuation: true },
        { character: '好', english: 'so' },
        { character: '涼快', english: 'cool' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-06',
      chinese: '我們住在三樓。',
      english: 'We live on the third floor.',
      words: [
        { character: '我們', english: 'we' },
        { character: '住', english: 'live', isVocab: true },
        { character: '在', english: 'on' },
        { character: '三', english: 'three' },
        { character: '樓', english: 'floor', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-07',
      chinese: '花園裡有很多漂亮的花。',
      english: 'There are many beautiful flowers in the garden.',
      words: [
        { character: '花園', english: 'garden', isVocab: true },
        { character: '裡', english: 'inside' },
        { character: '有', english: 'have' },
        { character: '很多', english: 'many' },
        { character: '漂亮', english: 'beautiful' },
        { character: '的', english: "'s" },
        { character: '花', english: 'flowers' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-08',
      chinese: '我喜歡在陽台上看風景。',
      english: 'I like to enjoy the view on the balcony.',
      words: [
        { character: '我', english: 'I' },
        { character: '喜歡', english: 'like' },
        { character: '在', english: 'on' },
        { character: '陽台', english: 'balcony', isVocab: true },
        { character: '上', english: 'on' },
        { character: '看', english: 'watch' },
        { character: '風景', english: 'view' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-09',
      chinese: '小心走樓梯，不要跑。',
      english: 'Be careful on the stairs, do not run.',
      words: [
        { character: '小心', english: 'careful' },
        { character: '走', english: 'walk' },
        { character: '樓梯', english: 'stairs', isVocab: true },
        { character: '，', english: '', isPunctuation: true },
        { character: '不要', english: 'do not' },
        { character: '跑', english: 'run' },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
    {
      id: 'sent-10',
      chinese: '我們買了新的傢俱放在客廳。',
      english: 'We bought new furniture to put in the living room.',
      words: [
        { character: '我們', english: 'we' },
        { character: '買', english: 'bought' },
        { character: '了', english: '(completed)' },
        { character: '新的', english: 'new' },
        { character: '傢俱', english: 'furniture', isVocab: true },
        { character: '放', english: 'put' },
        { character: '在', english: 'in' },
        { character: '客廳', english: 'living room', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
    },
  ],
  story: {
    title: {
      chinese: '新家',
      english: 'The New Home',
    },
    content: [
      [
        { character: '小美', english: 'Xiao Mei' },
        { character: '一家', english: 'family' },
        { character: '搬', english: 'move' },
        { character: '到', english: 'to' },
        { character: '新', english: 'new' },
        { character: '家', english: 'home' },
        { character: '，', english: '', isPunctuation: true },
        { character: '住', english: 'live', isVocab: true },
        { character: '在', english: 'on' },
        { character: '三', english: 'three' },
        { character: '樓', english: 'floor', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
      [
        { character: '小美', english: 'Xiao Mei' },
        { character: '很', english: 'very' },
        { character: '興奮', english: 'excited' },
        { character: '，', english: '', isPunctuation: true },
        { character: '說', english: 'said' },
        { character: '：', english: '', isPunctuation: true },
        { character: '「', english: '', isPunctuation: true },
        { character: '我', english: 'I' },
        { character: '要', english: 'want' },
        { character: '看', english: 'see' },
        { character: '每', english: 'every' },
        { character: '一', english: 'one' },
        { character: '個', english: '' },
        { character: '房間', english: 'room', isVocab: true },
        { character: '！', english: '', isPunctuation: true },
        { character: '」', english: '', isPunctuation: true },
      ],
      [
        { character: '她', english: 'she' },
        { character: '先', english: 'first' },
        { character: '跑', english: 'ran' },
        { character: '到', english: 'to' },
        { character: '客廳', english: 'living room', isVocab: true },
        { character: '，', english: '', isPunctuation: true },
        { character: '客廳', english: 'living room', isVocab: true },
        { character: '好', english: 'so' },
        { character: '大', english: 'big' },
        { character: '！', english: '', isPunctuation: true },
      ],
      [
        { character: '然後', english: 'then' },
        { character: '她', english: 'she' },
        { character: '走', english: 'walked' },
        { character: '樓梯', english: 'stairs', isVocab: true },
        { character: '上', english: 'up' },
        { character: '去', english: 'go' },
        { character: '看', english: 'see' },
        { character: '廚房', english: 'kitchen', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
      [
        { character: '媽媽', english: 'mom' },
        { character: '說', english: 'said' },
        { character: '：', english: '', isPunctuation: true },
        { character: '「', english: '', isPunctuation: true },
        { character: '小心', english: 'careful' },
        { character: '走', english: 'walk' },
        { character: '樓梯', english: 'stairs', isVocab: true },
        { character: '！', english: '', isPunctuation: true },
        { character: '」', english: '', isPunctuation: true },
      ],
      [
        { character: '小美', english: 'Xiao Mei' },
        { character: '打開', english: 'opened' },
        { character: '門', english: 'door', isVocab: true },
        { character: '，', english: '', isPunctuation: true },
        { character: '找到', english: 'found' },
        { character: '了', english: '' },
        { character: '浴室', english: 'bathroom', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
      [
        { character: '她', english: 'she' },
        { character: '看見', english: 'saw' },
        { character: '自己', english: 'her own' },
        { character: '的', english: "'s" },
        { character: '房間', english: 'bedroom', isVocab: true },
        { character: '，', english: '', isPunctuation: true },
        { character: '有', english: 'has' },
        { character: '大', english: 'big' },
        { character: '窗', english: 'window', isVocab: true },
        { character: '！', english: '', isPunctuation: true },
      ],
      [
        { character: '窗', english: 'window', isVocab: true },
        { character: '外面', english: 'outside' },
        { character: '可以', english: 'can' },
        { character: '看見', english: 'see' },
        { character: '花園', english: 'garden', isVocab: true },
        { character: '。', english: '', isPunctuation: true },
      ],
      [
        { character: '小美', english: 'Xiao Mei' },
        { character: '問', english: 'asked' },
        { character: '：', english: '', isPunctuation: true },
        { character: '「', english: '', isPunctuation: true },
        { character: '我們', english: 'we' },
        { character: '有', english: 'have' },
        { character: '陽台', english: 'balcony', isVocab: true },
        { character: '嗎', english: '(question)' },
        { character: '？', english: '', isPunctuation: true },
        { character: '」', english: '', isPunctuation: true },
      ],
      [
        { character: '爸爸', english: 'dad' },
        { character: '帶', english: 'took' },
        { character: '她', english: 'her' },
        { character: '去', english: 'to' },
        { character: '陽台', english: 'balcony', isVocab: true },
        { character: '看', english: 'see' },
        { character: '風景', english: 'view' },
        { character: '。', english: '', isPunctuation: true },
      ],
      [
        { character: '新', english: 'new' },
        { character: '傢俱', english: 'furniture', isVocab: true },
        { character: '明天', english: 'tomorrow' },
        { character: '就', english: 'will' },
        { character: '到', english: 'arrive' },
        { character: '了', english: '' },
        { character: '！', english: '', isPunctuation: true },
      ],
      [
        { character: '小美', english: 'Xiao Mei' },
        { character: '開心', english: 'happy' },
        { character: '地', english: '' },
        { character: '說', english: 'said' },
        { character: '：', english: '', isPunctuation: true },
        { character: '「', english: '', isPunctuation: true },
        { character: '我', english: 'I' },
        { character: '愛', english: 'love' },
        { character: '我們', english: 'our' },
        { character: '的', english: "'s" },
        { character: '新', english: 'new' },
        { character: '家', english: 'home' },
        { character: '！', english: '', isPunctuation: true },
        { character: '」', english: '', isPunctuation: true },
      ],
    ],
  },
  quiz: [
    {
      id: 'q1',
      type: 'multiple-choice',
      question: 'What does 客廳 mean?',
      options: ['bedroom', 'living room', 'kitchen', 'bathroom'],
      correctAnswer: 1,
    },
    {
      id: 'q2',
      type: 'multiple-choice',
      question: 'What does 廚房 mean?',
      options: ['living room', 'bedroom', 'kitchen', 'bathroom'],
      correctAnswer: 2,
    },
    {
      id: 'q3',
      type: 'multiple-choice',
      question: "Which word means 'door'?",
      options: ['窗', '門', '樓', '房間'],
      correctAnswer: 1,
    },
    {
      id: 'q4',
      type: 'matching',
      question: 'Match the rooms:',
      pairs: [
        { chinese: '客廳', english: 'living room' },
        { chinese: '廚房', english: 'kitchen' },
        { chinese: '浴室', english: 'bathroom' },
      ],
    },
    {
      id: 'q5',
      type: 'multiple-choice',
      question: '我們住在三樓 means:',
      options: ['We have three floors', 'We live on the third floor', 'We see three buildings', 'Three people live here'],
      correctAnswer: 1,
    },
    {
      id: 'q6',
      type: 'multiple-choice',
      question: 'What does 花園 mean?',
      options: ['balcony', 'stairs', 'garden', 'furniture'],
      correctAnswer: 2,
    },
    {
      id: 'q7',
      type: 'multiple-choice',
      question: "Which word means 'balcony'?",
      options: ['花園', '陽台', '樓梯', '傢俱'],
      correctAnswer: 1,
    },
    {
      id: 'q8',
      type: 'multiple-choice',
      question: 'What does 樓梯 mean?',
      options: ['floor', 'stairs', 'window', 'door'],
      correctAnswer: 1,
    },
    {
      id: 'q9',
      type: 'matching',
      question: 'Match the home words:',
      pairs: [
        { chinese: '花園', english: 'garden' },
        { chinese: '陽台', english: 'balcony' },
        { chinese: '樓梯', english: 'stairs' },
        { chinese: '傢俱', english: 'furniture' },
      ],
    },
    {
      id: 'q10',
      type: 'multiple-choice',
      question: '我喜歡在陽台上看風景 means:',
      options: ['I like the garden view', 'I like to enjoy the view on the balcony', 'I see flowers on the stairs', 'I put furniture on the balcony'],
      correctAnswer: 1,
    },
  ],
};

export default lesson;
